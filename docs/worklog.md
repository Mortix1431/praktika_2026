...
`obj=0x<контур>,pt=<getCenterPoint> cmdid=command_finish` — **ОДИН** finish,
точка — **центр** (`getCenterPoint`), команда `smshell` строкой.
Тело берём разницей тел до/после (`getObjectsByFilter`+`Subtract`), сверяем
`compareSolids(эталон, построенное)`.

Приглашения команды (командная строка): «Выберите 2D эскиз профиля или
[Закончить]» → «Выберите точку на контуре или [Закончить]». Зазор — как в
SmRuled: `cmdid=param_GapShiftType cmdid=enum_gs* num=<…>` либо
`cmdid=param_GapValue num=<…>`; «точка на контуре» = `cmdid=10009` + точка.

### ГЛАВНЫЙ вывод: источник должен быть ОБЫЧНЫМ замкнутым контуром
Наш самодельный шаблон использовал параметрический **«2D Эскиз»** из дерева
3D-построений, с разомкнутым контуром (`pol2d.isClosed=false` в каждом логе).
В тест-режиме с таким источником: выбор проходит (меню доходит до параметров),
но перестроение фичи между шагами команды не запускается — интерактивно его
двигает UpdateAll между кликами человека (~4 с), а `TestExecuteCommand` подаёт
ввод без прокачки апдейтов. На выходе команда откатывает недостроенную фичу:
в логе каждый прогон — `Erased: <handle>` и ни одного `rt_Success`, время
0.2 с против 13–25 с ручной сборки. Форс через `RegSaver("Nano3d")/Force3dCvt`
не помогает.

Во ВСЕХ шаблонах препода (SmHole/SmCreate/SmRuled) источники — **обычные
замкнутые полилинии в модели**, и те же команды строятся синхронно внутри
`TestExecuteCommand` (SmRuled — та же фича SmRuledSolid!). Поэтому:
- контрольный опыт: открыть `SmRuled_TemplateTest.dwg` (лежит рядом, в
  `AutoTests\Files\3D\Sheet Metal`), запустить `test_SmRuled` — механизм
  в сборке работает;
- пересобрать `SmShell_TemplateTest.dwg` по формату препода: источник —
  `_RECTANG` (замкнутая полилиния, НЕ «2D Эскиз»), эталон — обечайка,
  построенная вручную из копии того же контура; handle'ы — из Инспектора.
